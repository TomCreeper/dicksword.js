...
class Client extends EventEmitter
{
  ...
  _setEndpoints()
  {
    ...
    this.ENDPOINTS = {
      ...
      USER_REACTIONS: (channelID, messageID, reaction, userID) => {return `${this.ENDPOINTS.MESSAGE_REACTIONS(channelID, messageID, reaction)}/${!userID || userID === this.id ? '@me' : userID}`;},
  ...
  _handleWS(data)
  {
    ...
    case 'MESSAGE_REACTION_ADD':
      let react = {
        ...
        channel_id: wsData.channel_id,
        message_id: wsData.message_id,
        emoji: new Emoji(wsData.emoji)
      };
      this.emit('messageReactionAdd', react);
      break;
    case 'MESSAGE_REACTION_REMOVE':
      ...
      this.emit('messageReactionRemove', reactR);
      break;
    case 'MESSAGE_REACTION_REMOVE_ALL':
      ...
      this.emit('messageReactionRemoveAll', reactRA);
      break;
    case 'CHANNEL_CREATE':
      wsData.guild_id ? 
    ...
    case 'GUILD_ROLE_UPDATE':
      this.servers[wsData.guild_id].roles.addRole(wsData.role, this);
      this.emit('guildRoleUpdate', wsData);
      break;
    case 'GUILD_ROLE_CREATE':
      this.servers[wsData.guild_id].roles.addRole(wsData.role, wsData.guild_id, this);
      this.emit('guildRoleCreate', wsData);
      break;
    case 'GUILD_ROLE_DELETE':
      delete this.servers[wsData.guild_id].roles[wsData.role_id];
      this.emit('guildRoleDelete', wsData);
      break;
    case 'GUILD_MEMBER_ADD':
      this.servers[wsData.guild_id].members[wsData.user.id] = new Member(wsData, this);
    ...
    case 'GUILD_MEMBER_REMOVE':
      ...
      {
        if(this.servers[wsData.guild_id].members[wsData.user.id])
        {
  ...
  _handleGuildChunk(chunk)
  {
    ...
    for(let i = 0; i < arrLen; i ++)
    {
      ...
      if(!server.members[userID]) 
      {
        ...
        chunk.members[i].guild_id = chunk.guild_id;
        server.members[userID] = new Member(chunk.members[i], this.client);
      }
    }
  }
  /**
  ...
  _handleOauth()
  {
    ...
    {
      this.internals.oauth = oauth.body;
    }).catch((err) =>
    {
      emit('debug', err);
    });
  ...
  _handleWSClose(code, data)
  {
    ...
    console.log(`${code}: ${data} in WS CLOSE`);
  ...
  reconnect()
  {
    ...
    },8000 + (Math.pow(5000, this.internals.__reconnects)));
  }
  /**
   * Resets the reconnect counter every 24 hours since the bot started
   */
  _resetReconnects()
  {
    setInterval(() =>
    {
      this._internals._reconnects = 0;
    },8.64E7);
  }
  /**
  ...
  setPresence(presence)
  {
    ...
    return new Promise((resolve, reject) =>
    {
      ...
      currentPresence.since = typeof(presence.since) === 'number' ? presence.since : null;
      ...
      STATUS: (statusObj) =>
      {
        return {
          ...
          d: {
            ...
            since: typeof(statusObj.since) === 'number' ? Date.now() : null,
  ...
  uploadFile(channelID, file, message = {})
  {
    return new Promise((resolve, reject) =>
    {
      ...
      if(message.embed)
      {
        embed = encodeURIComponent(JSON.stringify(message.embed));
      }
      let content = message.content || '';
      let isBuff = Buffer.isBuffer(file);
      let isStr = typeof(file) === 'string';
      let upload = '';
      let fileName = '';
      if(!isBuff && !isStr)
      {
        reject('You need to supply a string or buffer to upload a file.');
      }
      else if(isBuff)
      {
        upload = file
      ...
      fileName = message.fileName ? message.fileName : fileName;
      form.append('content', content);
      form.append('file', upload, fileName);
      form.end();
      this._APIcall('post', this.ENDPOINTS.MESSAGES(channelID), form.toStream()).then((result) =>
      {
        resolve(new Message(result.body, this));
      }).catch(reject);
    });
  ...
  /**
   * Retrieves a message from Discord.
  ...
  getMessage(channelID, messageID)
  {
    return new Promise((resolve, reject) =>  
    {
      this._APIcall('get', this.ENDPOINTS.MESSAGES(channelID, messageID)).then((result) =>
      {
        resolve(new Message(result.body, this));
      }).catch(reject);
    });
  ...
  /**
   * Edits the info about he bot.
   * @param {Object} info - New User object to set the bot to. 
   */
  editInfo(info)
  {
    return new Promise((resolve, reject) =>
  ...
  /**
   * Gets the generic invite link for the bot.
   * @returns {String} Invite link for the bot (with no added permissions)
   */
  getBotInvite()
  {
    return `https://discordapp.com/oauth2/authorize?client_id=${this.id}&scope=bot`;
  }
  /**
...
class Server extends Base
{
  ...
  {
    super(data);
    data.roles ? this.roles  = new Roles(data.roles, this.id, client): '';
    data.members ? this.members = new Members(data.members, client, this.id) : '';
    data.channels ? this.channels = new Channels(data.channels, client, this.id) : '';
    Object.defineProperty(this, '_client',{enumerable: false, value: client});
  }
  /**
   * Creates a new role on the server
   * @param {Object} [roleData] Data for a role
   * @param {string} [roleData.name='new role'] Name of the role
   * @param {int} [roleData.permissions] Permissions to set for the role, defaults to "@everyone permissions of server"
   * @param {color} [roleData.color=0] RGB color value for the role
   * @param {boolean} [roleData.hoist=false] Whether or not the role is shown separately on the member list
   * @param {boolean} [roleData.mentionable=false] Whether or not the role is mentionable
   * @example
   * server.addRole({name:'Example role', hoist: true, color: 13395711});
   */
  addRole(roleData)
  {
    return new Promise((resolve, reject) =>
    {
      ...
      let serverID = this.id;
      this._client._APIcall('post', this._client.ENDPOINTS.ROLES(this.id),roleData ? roleData : {}).then((result) =>
      {
        resolve(result.body);
      }).catch(reject);
    });
...
class Members
{
  constructor(data, client, serverID)
  {
    let len = data.length;
    for(let i = 0; i < len; i++)
    {
      data[i].guild_id = serverID;
      this[data[i].user.id] = new Member(data[i], client);
...
class Member
{
  constructor(data, client)
  {
    this.fillData(data);
    Object.defineProperty(this, '_client',{enumerable: false, value: client});
    Object.defineProperty(this, 'guild_id',{enumerable: false, value: data.guild_id});
  }
  update(data)
  {
    this.fillData(data);
  ...
  fillData(data)
  {
    ...
    this.bot = data.user.bot ? true : false;
  }
  addToRole(roleID)
  {
    return new Promise((resolve, reject) =>
    {
      this._client._APIcall('put', this._client.ENDPOINTS.MEMBER_ROLES(this.guild_id, this.id, roleID)).then((result) =>
      {
        resolve(result.body);
      }).catch(reject);
    })
  ...
  removeFromRole(roleID)
  {
    return new Promise((resolve, reject) =>
    {
      this._client._APIcall('delete', this._client.ENDPOINTS.MEMBER_ROLES(this.guild_id, this.id, roleID)).then((result) =>
      {
        resolve(result.body);
      }).catch(reject);
    })
...
class Roles
{
  constructor(data, serverID, client)
  {
    ...
    for(let i = 0; i < len; i ++)
    {
      this[data[i].id] = new Role(data[i], serverID, client);
    }
  }
  addRole(data, serverID, client)
  {
    this[data.id] = new Role(data, serverID, client);
  }
  get [Symbol.toStringTag]()
  {
    return 'Roles';
...
/**
 * Contains everything pertaining to a Role
 * @param {Object} roleData Raw role data from the websocket
 */
class Role
{
  constructor(roleData, serverID, client)
  {
    Object.assign(this, roleData);
    Object.defineProperty(this, '_client',{enumerable: false, value: client});
    Object.defineProperty(this, 'serverID',{enumerable: false, value: serverID});
  }
  delete()
  {
    return new Promise((resolve, reject) =>
    {
      this._client._APIcall('delete', this._client.ENDPOINTS.ROLES(this.serverID, this.id)).then((result) =>
      {
        resolve(result.body);
      }).catch(reject);
    });
...
class Message
{
  ...
  edit(content)
  {
    return new Promise((resolve, reject) =>
    {
      ...
      this._client._APIcall('patch', this._client.ENDPOINTS.MESSAGES(this.channel_id, this.id), newContent).then((result) =>
      {
        resolve(result.body);
      }).catch(reject);
    });
   ...
   * @param {Object} emoji Either an object with the emoji name and ID as props or just an emoji symbol as a string
   * @returns {Promise<Object>} 
   */
  react(emoji)
  {
    return new Promise((resolve, reject) =>
    {
      this._client._APIcall('put', this._client.ENDPOINTS.USER_REACTIONS(this.channel_id, this.id, encodeURIComponent(new Emoji(emoji).toString()))).then((result) =>
      {
        resolve(result.body);
      }).catch(reject); 
    });
  ...
  /**
   * Cancel's the bots own reaction.
   * @param {Object | String} emoji Either an object with the emoji name and ID as props or just an emoji symbol as a string
   * @returns {Promise<Object>} 
   */
  cancelReact(emoji)
  {
    return new Promise((resolve, reject) =>
    {
      this._client._APIcall('delete', this._client.ENDPOINTS.USER_REACTIONS(this.channel_id, this.id, encodeURIComponent(new Emoji(emoji).toString()))).then((result) =>
      {
        resolve(result.body);
      }).catch(reject); 
    });
   ...
   * @returns {Promise<Object>} 
   */
  deleteReact(emoji, userID)
  {
    return new Promise((resolve, reject) =>
    {
      this._client._APIcall('delete', this._client.ENDPOINTS.USER_REACTIONS(this.channel_id, this.id, encodeURIComponent(new Emoji(emoji).toString()), userID)).then((result) =>
      {
        resolve(result.body);
      }).catch(reject); 
    });
  ...
  deleteReacts()
  {
    return new Promise((resolve, reject) =>
    {
      this._client._APIcall('delete', this._client.ENDPOINTS.MESSAGE_REACTIONS(this.channel_id, this.id)).then((result) =>
      {
        resolve(result);
      }).catch(reject); 
    });
  ...
  getReacts(emoji)
  {
    return new Promise((resolve, reject) =>
    {
      this._client._APIcall('get', this._client.ENDPOINTS.MESSAGE_REACTIONS(this.channel_id, this.id, encodeURIComponent(new Emoji(emoji).toString()))).then((result) =>
      {
        resolve(result.body);
      }).catch(reject); 
    });
...
/**
 * Emoji class represents an emoji
 * @param {Object | String} emoji Either an object with the emoji name and ID as props or just an emoji symbol as a string
 */
class Emoji
{
  constructor(emoji)
  {
    if(typeof(emoji) === 'object')
    {
      ...
      this.id = emoji.id ? emoji.id : null;
    }
    else
    {
      this.name = emoji;
